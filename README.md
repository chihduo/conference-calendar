...
還有第三種狀態:**灰字**。一個會議的投稿已經截止、而你沒有追蹤它的論文時，它後續的
notification、rebuttal、camera-ready 就是別人的行程表——灰掉而不是隱藏，因為知道某個
會議什麼時候放榜仍然值得一瞥。投稿與摘要本身永遠不灰（那才是你要決定的事），而滾動
截稿的會議只要還有下一輪可投就整屆維持正常。目前 193 個未來里程碑裡有 15 個屬於這類。

急迫度走左側色條、歸屬走背景色、無關走文字顏色，三個獨立通道疊加而不互相蓋掉。
沒有投稿紀錄時時間軸完全不變，不會多出任何雜訊。
